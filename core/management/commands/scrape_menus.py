...
class Command(BaseCommand):
    ...
    def update_tables(self, soup, location):
        ...
        for m in meals:
            ...
            menu = m.find_all("tr")[0].find_next_sibling("tr").find("table").find_all("tr", recursive=False)
            for dish in menu:
                if dish.find_all("div", class_="shortmenucats"):
                    # it's not a dish, it's the category of the following dishes
                    category = dish.find_all("div", class_="shortmenucats")[0].string.strip("- ").title()
                    # Late night is stored under Dinner, making things a bit more difficult!
                    if (category == "Late Night Grill") or (category == "Late Night"):
                        meal_name = "LN"
                    ...
                    contains_nuts = False
                    try:
                        price = dish.find("div", class_="shortmenuprices").find("span").string.strip("$")
                    except:
                        price = ""
